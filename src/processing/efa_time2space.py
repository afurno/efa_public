...
'''
 https://personality-project.org/r/html/fa.html
 Among the many ways to do latent variable exploratory factor analysis (EFA), one of the better is to use Ordinary Least Squares (OLS)
 to find the minimum residual (minres) solution. This produces solutions very similar to maximum likelihood even for badly behaved matrices.
 A variation on minres is to do weighted least squares (WLS). Perhaps the most conventional technique is principal axes (PAF). 
 An eigen value decomposition of a correlation matrix is done and then the communalities for each variable are estimated by the first n factors. 
 These communalities are entered onto the diagonal and the procedure is repeated until the sum(diag(r)) does not vary. Yet another estimate 
 procedure is maximum likelihood. For well behaved matrices, maximum likelihood factor analysis (either in the fa or in the factanal function)
 is probably preferred. Bootstrapped confidence intervals of the loadings and interfactor correlations are found by fa with n.iter > 1.
'''
...
def efa(nfact, rotation="none", method="ml", cor=True, period_min=60):
    ...
    signatures, signatures_df, _ = utilities.generate_signatures(traffic_matrix, traffic_snapshots, \
                                                 traffic_base_stations, loadings, factor_names, label, output_folder, method + "_" + rotation)
    ...
    signatures, signatures_df, _ = utilities.generate_signatures(traffic_matrix, traffic_snapshots, \
                                                 traffic_base_stations, loadings, factor_names, label, output_folder, method + "_" + rotation, threshold=0.4)
    ...
    signatures, signatures_df, _ = utilities.generate_signatures(traffic_matrix, traffic_snapshots, \
                                                 traffic_base_stations, loadings, factor_names, label, output_folder, method + "_" + rotation, threshold=0.6)
    ...
    signatures, signatures_df, _ = utilities.generate_signatures(traffic_matrix, traffic_snapshots, \
                                                 traffic_base_stations, loadings, factor_names, label, output_folder, method + "_" + rotation, threshold=0.8)
    ...
    loadings_df = analyze_loadings(loadings, factor_names, cell_ids, method + "_" + rotation)
...
def nnmf(nfact, method="scd", loss="mse", period_min=60):    
    ...
    cell_ids = np.asarray(ro.r('colnames(fit$H)'))
    print "Cell ids are: " + str(cell_ids)
    
    traffic_matrix = np.asarray(ro.r('x'))
    traffic_snapshots = np.asarray(ro.r('rownames(x)'))
    traffic_base_stations = np.asarray(ro.r('colnames(x)'))
    signatures, signatures_df, _ = utilities.generate_signatures(traffic_matrix, traffic_snapshots, \
                                                 traffic_base_stations, loadings, factor_names, label, output_folder, method + "_" + loss)
    ...
    signatures, signatures_df, _ = utilities.generate_signatures(traffic_matrix, traffic_snapshots, \
                                                 traffic_base_stations, loadings, factor_names, label, output_folder, method + "_" + loss, threshold=0.4, max_factor_value=True)
    ...
    signatures, signatures_df, _ = utilities.generate_signatures(traffic_matrix, traffic_snapshots, \
                                                 traffic_base_stations, loadings, factor_names, label, output_folder, method + "_" + loss, threshold=0.6, max_factor_value=True)
    ...
    signatures, signatures_df, _ = utilities.generate_signatures(traffic_matrix, traffic_snapshots, \
                                                 traffic_base_stations, loadings, factor_names, label, output_folder, method + "_" + loss, threshold=0.8, max_factor_value=True)
    ...
    loadings_df = analyze_loadings(loadings, factor_names, cell_ids, method + "_" + loss)
...
def analyze_loadings(loadings, factor_names, cell_ids, folder_prefix):
...
def analyze_uniquenesses(uniquenesses, cell_ids, folder_prefix):
    # create dataframe reporting for each snapshot the loading associated to each factor
...
num_snapshots = len(cells_df["time"].unique())
...
# print cells_df.head()
cells_df = cells_df.pivot(index='time', columns='cell-id', values='total_volume_per_day_of_the_week')
# print cells_df.head()
...
'''
# efa(snapshots_filename, nfact, rotation="none", method="ml", cor=cor, period_min = period_min)
# efa(snapshots_filename, nfact, rotation="varimax", method="pa", cor=cor, period_min = period_min)
# efa(snapshots_filename, nfact, rotation="promax", method="ml", cor=cor, period_min = period_min)
