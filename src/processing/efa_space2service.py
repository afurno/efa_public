...
def efa(nfact, rotation="none", method="ml", cor=True, period_min=60):
    ...
    print "*** Here follows the services names:"
...
def nnmf(nfact, method="scd", loss="mse", period_min=60):    
    ...
    service_names = np.asarray(ro.r('colnames(fit$H)'))
    print "Services are: " + str(service_names)
    ...
    loadings_df = analyze_loadings(loadings, factor_names, service_names, method + "_" + loss)
...
services_filename = output_folder + "/cells_matrix_for_service_profiling" + (
...
cells_df = cells_df.groupby(["category", "cell-id", "lon", "lat", "area"]).sum()
cells_df = cells_df.reset_index()
cells_df = cells_df[["category", "cell-id", "lon", "lat", "area", "total_volume_per_day_of_the_week"]]
print cells_df.head()
...
cells_df = cells_df.pivot(index='cell-id', columns='category', values='total_volume_per_day_of_the_week')
# print cells_df.head()

if not os.path.exists(os.path.dirname(services_filename)):
    os.makedirs(os.path.dirname(services_filename))
cells_df.to_csv(services_filename, index=True, header=True, sep=" ", index_label=False)
...
# read data matrix X
ro.r('x<-rbind(as.matrix(read.table(file="' + services_filename + '", sep=" ", header=TRUE, row.names=1)))')
...
'''
#efa(services_filename, nfact, rotation="oblimin", method="ml", cor=cor)

pca_princomp(services_filename, cor=cor)
pca_principal(services_filename, nfact, rotation="none", cor=cor)
pca_principal(services_filename, nfact, rotation="varimax", cor=cor)
pca_principal(services_filename, nfact, rotation="promax", cor=cor)
#pca_principal(services_filename, nfact, rotation="oblimin", cor=cor)

efa(services_filename, nfact, rotation="none", method="minres", cor=cor)
efa(services_filename, nfact, rotation="varimax", method="minres", cor=cor)
efa(services_filename, nfact, rotation="promax", method="minres", cor=cor)
#efa(services_filename, nfact, rotation="oblimin", method="minres", cor=cor)

efa(services_filename, nfact, rotation="none", method="pa", cor=cor)
efa(services_filename, nfact, rotation="varimax", method="pa", cor=cor)
efa(services_filename, nfact, rotation="promax", method="pa", cor=cor)

#efa(services_filename, nfact, rotation="oblimin", method="pa", cor=cor)
#efa(services_filename, nfact, rotation="oblimin", method="ml", cor=cor)
'''
