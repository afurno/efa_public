...
def efa(snapshot_filename, nfact, output_folder, rotation="none", method="ml", cor=True):
    ...
    for c in CATEGORIES:
        ...
        found_elements = [(i, (v.split(",")[0])) for i, v in enumerate(population_names) if searched_category in v]
    ...
    loadings_df = analyze_loadings(loadings, factor_names, traffic_snapshots, output_folder, method + "_" + rotation)

    print "Plotting loadings (" + method + " and rotation " + rotation + ")...\n"
    plot_factor_loadings(factor_names, loadings_df, output_folder, method + "_" + rotation, False)
    plot_factor_loadings(factor_names, loadings_df, output_folder, method + "_" + rotation, True)
    ...
    uniquenesses_df = analyze_uniquenesses(uniquenesses, uniquenesses_names, output_folder, method + "_" + rotation)

    print "Plotting uniquenesses (" + method + " and rotation " + rotation + ")...\n"
    plot_factor_loadings(["uniquenesses"], uniquenesses_df, output_folder, method + "_" + rotation, False, True)
...
def analyze_loadings(loadings, factor_names, snapshot_names, output_folder, folder_prefix):
...
def analyze_uniquenesses(uniquenesses, snapshot_names, output_folder, folder_prefix):
    # create dataframe reporting for each snapshot the loading associated to each factor
...
def plot_factor_loadings(factor_names, loadings_df, output_folder, folder_prefix, norm_colors, uniquenesses=False):
    # plot with various axes scales
    num_subplots = len(factor_names)
    page_counter = 1
    max_figs_per_line = 5
    max_rows_per_page = 5
    fig_basename = output_folder + "/" + folder_prefix + ("/profiles" if not uniquenesses else "/uniquenesses") + ("_" + label if label is not None else "") \
    ...
    for factor in factor_names:
        ...
        print("Type of the axis: " + str(type(axs[counter])))
        if norm_colors:
...
cells_df["cell-id_service"] = cells_df["cell-id"].map(str) + "," + cells_df["category"].map(str) + ","
...
# print cells_df.head()
cells_df = cells_df.pivot(index='cell-id_service', columns='time', values='total_volume_per_day_of_the_week')
# print cells_df.head()
...
#efa(snapshots_filename, nfact, rotation="none", method="ml", cor=cor)
try:
    efa(snapshots_filename, nfact, rotation="varimax", method="ml", cor=cor)
